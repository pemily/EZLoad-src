...
public class EZDate implements Comparable<EZDate> {
    ...
    public static EZDate parseYYYYMMDDDate(String date) {
        if (date == null) return null;
        if (date.length() != 8) return null;
        try {
            return new EZDate(Integer.parseInt(date.substring(0, 4)), Integer.parseInt(date.substring(4, 6)), Integer.parseInt(date.substring(6, 8)));
        }
        catch(NumberFormatException ne){
                return null;
        }
    ...
    public static EZDate parseYYYYMMDDDate(String date, char separator) {
        if (date == null) return null;
        String elem[] = date.trim().split(separator+"");
