...
public class CurrencyMap {
    ...
    public CurrencyMap reverse() {
        List<PriceAtDate> reversedFactors = new ArrayList<>(this.factors.getPrices().size());
        this.factors.getPrices().forEach(priceAtDate -> {
            Price p = new Price(1.0f/priceAtDate.getValue(), priceAtDate.isEstimated());
            PriceAtDate pd = new PriceAtDate(priceAtDate.getDate(), p);
            reversedFactors.add(pd);
        });
        return new CurrencyMap(this.to, this.from, reversedFactors);
    }
}
